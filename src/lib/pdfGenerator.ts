...
export function generatePdf(
  ...
  const addHeaderFooter = () => {
    ...
    pdf.text(`Page ${pdf.getNumberOfPages()}`, pageWidth / 2, pageHeight - margin / 1.5, { align: "center" });
  };
  ...
  const addPageBorder = () => {
    ...
    pdf.setLineWidth(1);
    pdf.rect(bMargin + 3, bMargin + 3, pageWidth - (bMargin + 3) * 2, pageHeight - (bMargin + 3) * 2);
  };

  const writeText = (text: string, opts: { size?: number; bold?: boolean; italic?: boolean; align?: "left" | "center" | "justify"; spacingAfter?: number; fullWidth?: boolean } = {}) => {
  ...
  // Title (full width)
  writeText(parsed.title, { size: (spec.titleSize / 48) * (bodySize * 2), bold: true, align: "center", fullWidth: true, spacingAfter: 8 });
  writeText(parsed.authors.join(", "), { size: bodySize + 1, align: "center", fullWidth: true, spacingAfter: 4 });
  if (parsed.affiliations.length) {
    writeText(parsed.affiliations.join(" • "), { size: bodySize - 1, italic: true, align: "center", fullWidth: true, spacingAfter: 14 });
  }

  if (parsed.abstract) {
    writeText("Abstract", { size: (spec.headingSize / 24) * (bodySize * 1.5), bold: true, fullWidth: true });
  ...
  for (const section of parsed.sections) {
    writeText(section.heading, { size: (spec.headingSize / 24) * (bodySize * 1.5), bold: true, spacingAfter: 4 });
    for (const block of section.blocks) {
      if (block.type === "paragraph") writeText(block.text);
      else if (block.type === "list") block.items.forEach((it, i) => writeText(`${block.ordered ? `${i + 1}.` : "•"} ${it}`));
  ...
  if (parsed.references.length) {
    writeText("References", { size: (spec.headingSize / 24) * (bodySize * 1.5), bold: true, spacingAfter: 4 });
